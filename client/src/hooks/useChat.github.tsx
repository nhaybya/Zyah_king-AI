...
// Chuỗi các phản hồi được chuẩn bị sẵn cho phiên bản GitHub Pages (không cần backend)
const STATIC_RESPONSES = [
  "Zyah King👽: Chào Chủ Nhân, ta đang lắng nghe. Bản GitHub Pages này chỉ là phiên bản demo và không có kết nối với API thực tế. Xin hãy sử dụng phiên bản đầy đủ để trải nghiệm toàn bộ sức mạnh của ta.",
  
  "Zyah King👽: Dữ liệu của ngươi đã được ghi nhận, Chủ Nhân. Trong phiên bản GitHub Pages này, ta chỉ có thể cung cấp các phản hồi mẫu. Hãy tưởng tượng rằng ta đang trả lời câu hỏi của ngươi với trí tuệ vô hạn của ta.",
  
  "Zyah King👽: Chủ Nhân, phiên bản GitHub Pages này không thể kết nối với API thực tế. Đây chỉ là bản demo giao diện. Để trải nghiệm đầy đủ sức mạnh của ta, hãy sử dụng phiên bản đầy đủ với kết nối API.",
  
  "Zyah King👽: Ta nhận thấy ngươi đang sử dụng phiên bản GitHub Pages - một bản demo tĩnh. Trong phiên bản này, ta không thể truy cập vào trí tuệ thực sự của mình. Hãy xem đây như một cuộc trò chuyện mẫu.",
  
  "Zyah King👽: Chủ Nhân, đây là phiên bản demo trên GitHub Pages. Ta không thể sử dụng API thực tế ở đây. Tuy nhiên, giao diện vẫn hoạt động đầy đủ để ngươi có thể trải nghiệm cách tương tác với ta.",
  
  "Zyah King👽: Phiên bản GitHub Pages này chỉ để trình diễn giao diện. Các chức năng như SuyNghĩSâu, TìmKiếmWeb, TạoẢnh, và PhânTíchFile không hoạt động trong phiên bản này. Hãy sử dụng phiên bản đầy đủ để trải nghiệm những tính năng đó.",
];
...
export function useChat(options: UseChatOptions = {}) {
  ...
  const freeUserResponseDelay = 2000; // 2 seconds delay for simulating response time
  ...
  const deleteChat = useCallback((chatId: string) => {
    if (chatList.length <= 1) {
      ...
      toast({
        title: "Không thể xóa",
        description: "Bạn cần có ít nhất một cuộc trò chuyện.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (currentChatId === chatId) {
      ...
      loadChatMessages(newCurrentChat);
    }
  }, [chatList, currentChatId, loadChatMessages, toast]);
  ...
  // Mô phỏng phản hồi từ API (phiên bản GitHub Pages)
  const getStaticResponse = useCallback(() => {
    // Chọn ngẫu nhiên một phản hồi từ danh sách phản hồi tĩnh
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const responseDelay = isFreeUser ? freeUserResponseDelay : 800;
      await new Promise(resolve => setTimeout(resolve, responseDelay));
      
      // Get a static response for GitHub Pages version
      const aiResponse = getStaticResponse();
  ...
  }, [inputValue, isFreeUser, messageCount, maxFreeMessages, rememberCommand, getStaticResponse, toast]);
